...
The '%' represents any number of characters: zero, one, or more. For example, 
if you write a pattern '%s%', the strings 's', 'toaster', 'string', and 'cats' will all match this pattern.

The '_' represents exactly one character.
Thus, if you write pattern 's_', the string 'so' will match this pattern, but the strings 's' and 'soap' will not.
...
WHERE
 ...
 --EXISTS operator
 /*
The EXISTS operator checks if the subquery returns any records or not. If the subquery returns any records,
the EXISTS operator will return TRUE, otherwise, it will return FALSE.
...
WHERE
    product = 'milk'
    AND NOT EXISTS -- returns true if query do not returns any value and returns false if query returns any result
(SELECT
    supplier
FROM
    suppliers 
